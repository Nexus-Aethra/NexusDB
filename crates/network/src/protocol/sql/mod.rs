...
pub use ast::*;
pub use optimizer::{
    OrEqInLeaf, as_disjuncts, is_always_false, is_always_true, negate_cond, normalize_pred,
    normalize_pred_cond, or_eq_to_in,
};
pub use parser::{bind_params, parse, parse_prepared, split_sql_statements};
...
mod tests {
    ...
    fn create_if_not_exists() {
        ...
        let SqlStmt::CreateTable { if_not_exists, .. } = s else {
            panic!()
        };
        assert!(!if_not_exists);
    }
    ...
    fn compat_create_index_extension_drop() {
        ...
        assert!(
            matches!(s, SqlStmt::CreateIndex { table, cols, if_not_exists: true }
        ...
        let s = parse(
            b"ALTER TABLE t ADD COLUMN IF NOT EXISTS is_begin BOOLEAN NOT NULL DEFAULT false",
        )
        .unwrap();
        assert!(matches!(
            s,
    ...
    fn compat_jsonb_projection() {
        ...
        let SqlStmt::Select { items, .. } = s else {
            panic!("expected Select")
        };
        assert!(matches!(
            &items[0],
            SelectItem::Expr {
                expr: ScalarExpr::JsonGet { as_text: false, .. },
                ..
            }
        ));
        let s = parse(b"SELECT j->>'a' FROM t").unwrap();
        ...
        assert!(matches!(
            &items[0],
            SelectItem::Expr {
                expr: ScalarExpr::JsonGet { as_text: true, .. },
                ..
            }
        ));
        // 链式 j->'a'->>'b'
        ...
        let SqlStmt::Select { conds, .. } = s else {
            panic!("expected Select")
        };
        assert!(matches!(
            conds,
    ...
    fn compat_auto_rowid() {
        ...
        let SqlStmt::CreateTable { schema, .. } = s else {
            panic!("expected CreateTable")
        };
        assert_eq!(
            schema.columns[schema.pk_col as usize].name, "a",
            "{schema:?}"
        );
    ...
    fn compat_split_sql_statements() {
        ...
        // dollar-quote 内分号不分割
        let parts = split_sql_statements(
            "CREATE FUNCTION f() RETURNS TRIGGER AS $$ BEGIN RETURN NEW; END; $$ LANGUAGE plpgsql; SELECT 1;",
        );
    ...
    fn create_errors() {
        ...
        assert!(
            parse(b"CREATE TABLE t (a WAT PRIMARY KEY)")
                .unwrap_err()
    ...
    fn insert_roundtrip() {
        ...
        let SqlStmt::Insert { table, cols, rows } = s else {
            panic!()
        };
        assert_eq!(table, "t");
        ...
        let SqlStmt::Insert { cols, rows, .. } = s else {
            panic!()
        };
        ...
        assert!(
            parse(b"INSERT INTO t VALUES (1), (2, 3)")
                .unwrap_err()
    ...
    fn select_roundtrip() {
        ...
        assert_eq!(
            ...
            &Cond {
                col: "c".into(),
                op: CmpOp::Lt,
                val: SqlValue::Str(b"x".to_vec()),
                set: vec![]
            }
        );
        // 无 WHERE / 无 LIMIT
        let s = parse(b"SELECT * FROM t").unwrap();
        let SqlStmt::Select { conds, limit, .. } = s else {
            panic!()
        };
    ...
    fn select_errors() {
        ...
        let SqlStmt::Select { items, conds, .. } = s else {
            panic!()
        };
        assert_eq!(
            items,
        ...
        let s = parse(b"SELECT * FROM t WHERE c LIKE 'ab%' ORDER BY a DESC, b LIMIT 3 OFFSET 6")
            .unwrap();
        let SqlStmt::Select {
            conds,
            order,
        ...
        assert!(
            parse(b"SELECT * FROM t WHERE c LIKE '%ab'").is_err(),
            "仅前缀模式"
        );
        ...
        let SqlStmt::Update { sets, conds, .. } = s else {
            panic!()
        };
        assert_eq!(sets.len(), 2);
        assert_eq!(conds.as_conjuncts().unwrap().len(), 1);
        assert_eq!(
            parse(b"DROP TABLE t").unwrap(),
            SqlStmt::DropTable { table: "t".into() }
        );
        assert!(
            parse(b"SELECT * FROM t WHERE a = NULL")
                .unwrap_err()
                .contains("NULL")
        );
    ...
    fn prepared_params() {
        ...
        assert_eq!(n, 2);
        let SqlStmt::Insert { ref rows, .. } = s else {
            panic!()
        };
        ...
        assert_eq!(
            rows[0],
            vec![SqlValue::Int(7), SqlValue::Str(b"x".to_vec())]
        );
        // $n 显式编号 (乱序引用)
        ...
        let SqlStmt::Select { conds, .. } = bound else {
            panic!()
        };
        let cj = conds.as_conjuncts().unwrap();
        ...
        let SqlStmt::Update { sets, conds, .. } = bound else {
            panic!()
        };
        assert_eq!(sets[0].1, SqlValue::Int(1));
        assert_eq!(
            conds.as_conjuncts().unwrap()[0].set,
            vec![SqlValue::Int(2), SqlValue::Int(3)]
        );
        // 个数不符 / 混用 / simple query 拒绝 / LIMIT 位置拒绝
        assert!(
            bind_params(&s, &[SqlValue::Int(1)])
                .unwrap_err()
                .contains("missing")
        );
        assert!(parse_prepared(b"SELECT * FROM t WHERE a = ? AND b = $1").is_err());
        assert!(
            parse(b"SELECT * FROM t WHERE a = ?")
                .unwrap_err()
                .contains("prepared")
        );
        assert!(
            parse_prepared(b"SELECT * FROM t LIMIT ?").is_err(),
            "LIMIT 占位不支持"
        );
    }
}
