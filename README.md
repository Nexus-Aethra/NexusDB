...
- **JOIN (多表等值)**: N 表左深 `INNER|LEFT|RIGHT|FULL|CROSS JOIN ... ON/USING` — worker 完成点 hash join (shard 只本地扫描, 零跨线程), 多条件 ON + 非等值残余, 谓词/投影下推 + 索引驱动 gather, 表别名/限定列/ORDER/LIMIT; mysql+pg 驱动实机一致 (OR/子查询分阶段后续)
